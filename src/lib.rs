...
use viz::{
    ...
    Body, Bytes, Error, Handler, IntoResponse, Request, RequestExt, Response, ResponseExt, Result,
    Router,
};
...
pub async fn generate_request_parts(req: Request) -> RequestParts {
    ...
    let body = BodyExt::collect(body).await.unwrap_or_default().to_bytes();
    RequestParts {
        method: parts.method,
        uri: parts.uri,
...
async fn generate_response(res_options: ResponseOptions, rx: Receiver<String>) -> Result<Response> {
    let mut stream = Box::pin(rx.map(|html| Ok::<_, std::io::Error>(Bytes::from(html))));
...
fn provide_contexts(path: String, req_parts: RequestParts, default_res_options: ResponseOptions) {
