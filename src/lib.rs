...
use server_fn::{
    codec::Encoding, initialize_server_fn_map, middleware::BoxedService, redirect::REDIRECT_HEADER,
...
use viz::{
    types::RouteInfo, Body, Bytes, Error, Handler, IntoResponse, Request, RequestExt, Response,
    Result, Router,
};

type LazyServerFnMap<Req, Res> = Lazy<DashMap<&'static str, ServerFnTraitObj<Req, Res>>>;

static REGISTERED_SERVER_FUNCTIONS: LazyServerFnMap<Request, Response> =
    initialize_server_fn_map!(Request, Response);
...
where
    T: ServerFn<ServerRequest = Request, ServerResponse = Response> + 'static,
{
...
/// A Viz handler that responds to a server function request.
pub async fn handle_server_fn(req: Request) -> Response {
    let path = req.uri().path();

    if let Some(mut service) = get_server_fn_service(path) {
        service.0.run(req).await
    } else {
        Response::builder()
            .status(StatusCode::BAD_REQUEST)
            .body(Body::from(Full::from(format!(
...
pub fn get_server_fn_service(path: &str) -> Option<BoxedService<Request, Response>> {
...
pub fn generate_request_and_parts(req: Request<Body>) -> (Request<Body>, Parts) {
...
pub async fn handle_server_fns(req: Request) -> Result<Response> {
    handle_server_fns_inner::<fn()>(req).await
}
...
/// An Viz handlers to listens for a request with Leptos server function arguments in the body,
/// run the server function if found, and return the resulting [Response].
///
/// This can then be set up at an appropriate route in your application:
///
...
/// - [ResponseOptions]
#[tracing::instrument(level = "trace", fields(error), skip_all)]
pub async fn handle_server_fns_with_context<F>(req: Request) -> Result<Response>
where
    F: Fn() + Clone + Send + Sync + 'static,
{
    handle_server_fns_inner::<F>(req).await
}

async fn handle_server_fns_inner<F>(req: Request) -> Result<Response>
where
    F: Fn() + Clone + Send + Sync + 'static,
{
    let additional_context = req.state::<F>().unwrap();
    ...
    spawn_task!(async move {
        ...
        let res = if let Some(mut service) = get_server_fn_service(&path) {
            ...
            let mut res = service.0.run(req).await;
...
///
///     // build our application with a route
///     let app = Router::new().any("*", leptos_viz::render_app_to_stream(
///         leptos_options,
///         || view! { <MyApp/> },
///     ));
///
...
) -> impl Fn(Request) -> Pin<Box<dyn Future<Output = Result<Response>> + Send + 'static>>
       + Clone
       + Send
       + 'static
where
...
///     // build our application with a route
///     let app =
///         Router::new().any("*", leptos_viz::render_app_to_stream_in_order(
///             leptos_options,
///             || view! { <MyApp/> },
///         ));
///
...
/// to route it using [leptos_router], serving an HTML stream of your application.
///
/// This version allows us to pass Viz State/Extension/Extractor or other info from Viz or network
...
#[tracing::instrument(level = "trace", fields(error), skip_all)]
async fn generate_response(res_options: ResponseOptions, rx: Receiver<String>) -> Result<Response> {
    ...
    let mut res = Response::new(Body::from_stream(
...
#[tracing::instrument(level = "trace", fields(error), skip_all)]
fn provide_contexts(path: String, parts: Parts, default_res_options: ResponseOptions) {
...
///
///     // build our application with a route
///     let app = Router::new().any("*", leptos_viz::render_app_async(
///         leptos_options,
///         || view! { <MyApp/> },
///     ));
///
...
/// Returns an Viz [Handler](viz::Handler) that listens for a `GET` request and tries
/// to route it using [leptos_router], asynchronously rendering an HTML page after all
/// `async` [Resource](leptos::Resource)s have loaded.
///
/// ## Provided Context Types
    ...
    move |req: Request| {
        Box::pin({
            ...
            async move {
                ...
                let complete_stream = futures::stream::iter([Ok(Bytes::from(html))]);
    ...
    move |req: Request| {
        Box::pin({
            ...
            async move {
                ...
                let mut res = Response::new(Body::from(Full::from(html)));
...
/// Generates a list of all routes defined in Leptos's Router in your app. We can then use this to automatically
/// create routes in Viz's Router without having to use wildcard matching or fallbacks. Takes in your root app Element
/// as an argument so it can walk you app tree. This version is tailored to generate Viz compatible paths. Adding excluded_routes
/// to this function will stop `.leptos_routes()` from generating a route for it, allowing a custom handler. These need to be in Viz path format
#[tracing::instrument(level = "trace", fields(error), skip_all)]
    ...
    IV: IntoView + 'static,
{
    ...
    spawn_task!(async move {
        leptos_router::build_static_routes(&options, app_fn, &routes, &static_data_map)
            .await
...
pub trait LeptosRoutes {
    ...
    fn leptos_routes_with_handler<H, O>(self, paths: Vec<RouteListing>, handler: H) -> Self
    where
        ...
        match res {
            StaticResponse::ReturnResponse {
                ...
                *res.status_mut() = match status {
                    ...
                    StaticStatusCode::InternalServerError => StatusCode::INTERNAL_SERVER_ERROR,
                };
                res
            }
            StaticResponse::RenderDynamic => {
                let res =
                    render_dynamic(&path, &options, app_fn.clone(), additional_context.clone())
                        .await;
                handle_static_response(path, options, app_fn, additional_context, res).await
            }
            StaticResponse::RenderNotFound => {
                let res = not_found_page(tokio::fs::read_to_string(not_found_path(&options)).await);
                handle_static_response(path, options, app_fn, additional_context, res).await
            }
            StaticResponse::WriteFile { body, path } => {
                ...
                if let Err(e) = std::fs::write(&path, &body) {
                    tracing::error!("encountered error {} writing file {}", e, path.display());
                }
                    ...
                    async move {
                        ...
                        spawn_task!(async move {
                            let res = incremental_static_route(
                                tokio::fs::read_to_string(static_file_path(&options, &path)).await,
                            );
                        ...
                        rx.await
                            .map(|res| res.map(|body| Body::from(Full::from(body))))
                            .map_err(Error::boxed)
                    }
                })
            };
        ...
        IV: IntoView + 'static,
    {
        ...
        for (path, method) in server_fn_paths() {
            ...
            let handler = move |mut req: Request| {
                let _ = req.set_state(additional_context.clone());
                async move { handle_server_fns_with_context::<fn()>(req).await }
            };
            router = match method {
                Method::GET => router.get(path, handler),
                Method::POST => router.post(path, handler),
        ...
        O: IntoResponse + Send + 'static,
    {
        ...
        for listing in paths.iter() {
            for method in listing.methods() {
                let path = listing.path();
                router = match method {
                    leptos_router::Method::Get => router.get(path, handler.clone()),
                    leptos_router::Method::Post => router.post(path, handler.clone()),
                    leptos_router::Method::Put => router.put(path, handler.clone()),
                    leptos_router::Method::Delete => router.delete(path, handler.clone()),
                    leptos_router::Method::Patch => router.patch(path, handler.clone()),
                };
            }
        }
        router
